...
def _make_grpc_stub(resource_handle):
    ...
    return rand_num_grpc.RandomNumberServiceStub(
        intercepted_channel
    )
...
class GetRandomNumber(skill_interface.Skill):
    ...
    def execute(
        self,
        request: skill_interface.ExecuteRequest[get_rand_num_proto.GetRandomNumberParams],
        ...
        try:
            service_result = stub.GetRandomNumber(rand_num_proto.RandomNumberRequest(
                range_start=request.params.range_start,
                range_end=request.params.range_end,
            ))
        except grpc.RpcError as e:
            raise skill_interface.SkillError(code=e.code().value[0], message=e.details())

        logging.info(f'Result: {service_result.result}')
        
        return get_rand_num_proto.GetRandomNumberResult(result=service_result.result)
