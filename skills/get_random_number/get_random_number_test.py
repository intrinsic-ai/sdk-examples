...
class FakeRandomNumberServicer(rand_num_grpc.RandomNumberServiceServicer):
  ...
  def GetRandomNumber(
    ...
    return rand_num_proto.RandomNumberResponse(
        result=request.range_start + request.range_end
    )
...
class GetRandomNumberTest(unittest.TestCase):
  ...
  def test_execute(self):
    ...
    rand_num_grpc.add_RandomNumberServiceServicer_to_server(
        FakeRandomNumberServicer(), server
    )
    server.start()
