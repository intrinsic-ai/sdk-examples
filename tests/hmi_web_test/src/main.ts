...
async function forceCleanup() {
  ...
  const ops = (opsResp as any)?.operations || [];
  ...
  try {
    ...
    const create =
      await ExecutiveServiceService.executiveServiceCreateOperation({
        behaviorTree: treeToRun,
      } as any);
    const opName = (create as any).name;
    ...
    log(`State: ${(final as any).metadata?.operationState}`);
