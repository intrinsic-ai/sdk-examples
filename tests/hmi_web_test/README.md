...
2.  **Generate the client code**
    This project does not commit the generated client code.
    1. For generating the `openapi.yaml` file

       1.1 Export <YOUR_ORG> and <CTX>:

       ```bash
       export ORG="intrinsic@..."  # e.g intrinsic@intrinsic-prod-us
       export CTX="vmp-..."
       ```

       1.2 Download the OpenAPI Spec: Use the curl command to fetch the YAML definition directly from the web proxy.
...
1.  **Generate an Access Token:**
    ...
    ```bash
    export TOKEN=$(inctl auth print-access-token --org $ORG)
    ```
